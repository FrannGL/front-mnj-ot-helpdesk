...
export const generateOrderPdf = (order: {
  ...
  y += 12;
  doc.setFontSize(12);
  addLine('Fecha de solicitud', formatDate(order.created_at));
  addLine('Solicitante', order.cliente.username);

  // ✅ Renderizar categoría solo si hay tags
  if (order.tags.length > 0) {
    addLine('Categoría', order.tags.map((tag) => tag.tag).join(', '));
  }

  addLine('Prioridad', order.prioridad_display);

  // Descripción destacada
  y += 10;
  doc.setFont('helvetica', 'bold');
  doc.text('Descripción del trabajo a realizar:', 20, y);
  y += 8;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text('Agentes asignados:', 20, y);

  y += 7;
  doc.setFont('helvetica', 'normal');
  order.agentes.forEach((agente) => {
    doc.text(`- ${agente.username}`, 25, y);
    y += 6;
  });
